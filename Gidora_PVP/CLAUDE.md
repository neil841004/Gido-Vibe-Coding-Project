...
- `comboRush`：蓄滿後改為爆衝連擊，固定以基礎最高速度 1.5 倍向前衝，衝刺期間轉向倍率使用 `CONFIG.combo.rushTurnMultiplier`；首次撞到敵人或另一隻龍後鎖定目標、隱藏本體，播放多段快閃攻擊。
- `comboRefractBeam`：蓄滿後改為折光追獵炮，沿用光束炮蓄力/施放節奏，但使用分段光束視覺呈現微追蹤扭曲，射程使用 `CONFIG.combo.refractBeamRangeMultiplier`，命中額外失衡值使用 `CONFIG.combo.refractBeamStaggerBonusPct`。
